...
# タイトル変更時にルールを再評価するか
# trueにすると、ブラウザのタブ切替などでタイトルが変わった際に、
# ウィンドウが新しいルールにマッチするかどうかを再チェックします。
...
#### ヒント: タイトルが後から変わるウィンドウの扱い

- Chromeなどのブラウザでポップアップを開いた際など、ウィンドウ作成時には仮のタイトル（例：「無題 - Chrome」）が表示され、読み込み完了後に本来のタイトルに変わることがあります。
- このようなウィンドウを正しくルールにマッチさせるには、`[global]`設定の `recheck_on_title_change = true` を有効にしてください。
- これにより、ウィンドウタイトルが変更されたタイミングでルールが再評価され、確定後のタイトルに対して意図したルールを適用できます。
